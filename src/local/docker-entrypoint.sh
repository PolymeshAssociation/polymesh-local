...
then
  ...
  BOOT_NODE=--bootnodes=/ip4/${ALICE_IP}/tcp/30334/p2p/${BOOT_NODE_PEER_ID}
...
--no-prometheus --no-telemetry --pruning=archive --no-mdns --validator --rpc-cors=all --rpc-methods=unsafe --force-authoring --port 30333 --chain dev $1 ${BOOT_NODE}